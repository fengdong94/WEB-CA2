...
// render all FAQ items to page when page loaded
window.addEventListener("load", function () {
  FAQData.forEach(renderFAQItem);
});

// only render FAQ items according to the selected type
function filterFAQ(type) {
  ...
  if (oldActivedButton) {
    // clear old button active class
    oldActivedButton.className = "btn filter-btn";
  }

  if (oldActivedButton && type === oldActivedButton.id) {
    // cancel the selection, render all items
    FAQData.forEach(renderFAQItem);
  } else {
